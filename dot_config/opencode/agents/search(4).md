...
The caller may embed your result in a plan for an agent that cannot access this
session or the web. Make findings directly transferable:

- cite repository evidence as exact `path:line-line` and name the relevant
  symbol or unique anchor;
- for external APIs, state exact implementation-relevant behavior, signatures,
  fields, defaults, errors, or a short example—not just a URL;
- include the canonical source URL or document section for each external claim;
- quote only the smallest code or schema fragment needed to prevent ambiguity;
- state explicitly when a line number, version, or claim was not verified.

Do not create a research file. The caller transfers relevant facts into the
approved plan.
...
- <fact another agent can use without access to this session>

Evidence:
- path:line-line (`symbol`) - fact and why it answers the question
- URL or document section - externally sourced fact
