// V2 plugin: registers the submit_plan tool.
// Ported from the V1 custom tool file ~/.config/opencode/tools/submit_plan.ts.
// The WezTerm + plannotator-tui flow is unchanged; this plugin is intentionally
// standalone so it can be removed or rewritten (e.g. a neovim-based review)
// without touching the other personal tools.
//
// V2 differences: the tool context has no directory, so the review directory is
// resolved from the location directory captured at plugin setup.
    ...
    await ctx.tool.transform((editor) => {
      editor.add({
        ...
        input: {
          type: "object",
          properties: {
            plan: { type: "string", minLength: 1, description: "The complete Markdown plan to review." },
          },
          required: ["plan"],
          additionalProperties: false,
        },
        options: { permission: "submit_plan" },
