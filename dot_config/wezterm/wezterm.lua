...
-- Custom tab titles. Only the first format-tab-title handler runs, so this
-- replaces WezTerm's default renderer entirely, mirroring the stock
-- "N: <title>" labels with a leading/trailing space on every string so the
-- retro tab-bar labels do not concatenate. Precedence: special dotfiles-submenu
-- repo tabs show "N: <type>" (e.g. "2: wezterm") with no process suffix;
-- renamed tabs show "N: <name>"; tabs in a ":dotfiles" workspace show
-- "N: chezmoi" unless they are special or renamed; everything else shows
-- "N: <title>".
wezterm.on("format-tab-title", function(tab, _, _, _, _, _)
	local index = tab.tab_index + 1
	local dotfiles = wezterm.GLOBAL.dotfiles_tab_types
	local repo_type = dotfiles and dotfiles[tostring(tab.tab_id)]
	if repo_type then
		return string.format(" %d: %s ", index, repo_type)
	elseif tab.tab_title ~= "" then
		return string.format(" %d: %s ", index, tab.tab_title)
	else
		local mux_win = wezterm.mux.get_window(tab.window_id)
		local workspace = mux_win and mux_win:get_workspace() or ""
		if workspace:match(":dotfiles$") then
			return string.format(" %d: chezmoi ", index)
		end
