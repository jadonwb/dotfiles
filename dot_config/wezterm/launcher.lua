	...
	local function add_repo(name, dir)
		add(styled(name, "Green"), function(window, pane)
			local tab, new_pane = window:mux_window():spawn_tab({
				domain = {
					DomainName = pane:get_domain_name(),
				},
			})

			-- Record the repo type for this tab so the format-tab-title
			-- hook in wezterm.lua can render "N: <type>: <title>". Stored
			-- in GLOBAL (not tab:set_title) so it never collides with the
			-- rename submenu. Reassigning the whole GLOBAL table after
			-- mutation makes WezTerm persist the change.
			if tab then
				wezterm.GLOBAL.dotfiles_tab_types = wezterm.GLOBAL.dotfiles_tab_types or {}
				wezterm.GLOBAL.dotfiles_tab_types[tostring(tab:tab_id())] = name
				wezterm.GLOBAL = wezterm.GLOBAL
			end

			-- spawn_tab cannot pass cwd without the env leak noted in
			-- open_session, so initialize the new pane by hand.
			wezterm.time.call_after(0.1, function()
				if not new_pane then
					return
				end

				new_pane:send_text("cd " .. shell_path(dir) .. " && clear\n")
			end)
		end)
	end
...
local function build_main()
	...
	-- Inside the dotfiles session, offer its repo picker first.
	if active_workspace:match(":dotfiles$") then
		add(styled("dotfiles →", "Teal", true), function(window, pane)
			M.dotfiles(window, pane)
		end)
	end
...
function M.dotfiles(window, pane)
	show(window, pane, "dotfiles", build_dotfiles)
end
